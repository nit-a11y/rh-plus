/**
 * Script para Ler Dados de Desligados em Formato de Tabela
 * Lê o arquivo com formato de tabela estruturada
 */
...
/**
 * Ler arquivo de dados dos desligados em formato de tabela
 */
function readTableDisconnectedFile() {
    ...
    // Procurar linhas que começam com data de admissão
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        
        // Verificar se começa com data de admissão (formato DD/MM/AAAA)
        if (line.match(/^\d{2}\/\d{2}\/\d{4}/)) {
            ...
            if (parts.length >= 4) {
                const employee = {
                    ...
                    daysWorked: parts[2],
                    dismissalType: parts[3] || ''
                };
                
                // Continuar lendo as próximas linhas para encontrar os dados completos
                let j = i + 1;
                let fullText = line;
                
                // Acumular texto das próximas linhas até encontrar outra data ou fim
                while (j < lines.length && !lines[j].trim().match(/^\d{2}\/\d{2}\/\d{4}/)) {
                ...
                if (birthMatch && birthMatch[1] !== employee.admissionDate && birthMatch[1] !== employee.dismissalDate) {
                    employee.birthDate = birthMatch[1];
                }
                ...
                // Extrair nome (procurar padrão de nome em maiúsculas antes do CPF)
                ...
                // Extrair setor
                const sectorPatterns = ['Administrativo', 'Manutenção', 'Comercial', 'Marketing', 'Financeiro', 'Controladoria', 'Logística', 'Serviços Diversos'];
                ...
                i = j - 1; // Pular para o próximo bloco
            }
        }
    }
...
/**
 * Encontrar correspondências entre arquivo e banco
 */
async function findMatches() {
    console.log('🔧 Iniciando leitura do arquivo em formato de tabela...\n');
    
    try {
        // 1. Ler arquivo
        const fileEmployees = readTableDisconnectedFile();
        ...
        fileEmployees.forEach(fileEmp => {
            ...
            for (const dbEmp of dbEmployees) {
                ...
                // Busca por similaridade (mesmo primeiro nome)
                const fileFirstWord = normalizedFileName.split(' ')[0];
                const dbFirstWord = normalizedDbName.split(' ')[0];
                
                if (fileFirstWord === dbFirstWord && fileFirstWord.length > 3) {
                    matches.push({ file: fileEmp, db: dbEmp });
                    break;
                }
            }
        ...
        return { fileEmployees, matches, dbEmployees };
 ...
 */
async function executeUpdates(matches) {
    let updatedCount = 0;
...
async function main() {
    try {
        ...
        const { fileEmployees, matches, dbEmployees } = await findMatches();
        
        // Executar atualizações
        const updatedCount = await executeUpdates(matches);
        
        console.log(`\n📈 Total de atualizações realizadas: ${updatedCount}`);
        
        console.log('\n🎉 Processo concluído!');
        console.log('\n📊 Resumo:');
        console.log(`   - Total no arquivo: ${fileEmployees.length}`);
        console.log(`   - No banco de dados: ${dbEmployees.length}`);
