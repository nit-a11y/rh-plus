/**
 * Script para Debug da Contagem de Colaboradores - Versão 2
 */
...
async function debugEmployeeCount() {
    console.log('🔧 Debug da contagem de colaboradores v2...\n');
    ...
    try {
        ...
        const periodResult = await query(`
            ...
            WHERE c.name = $1
              AND (
                (e."admissionDate" <= $2 AND (e.type IS NULL OR e.type != 'Desligado'))
                OR
        ...
        // 7. Testar a função original
        console.log('\n7. Testando a função countActiveEmployees:');
        const { countActiveEmployees } = require('./overtime.js');
        
        const testCount1 = await countActiveEmployees(null, '2026', null);
        console.log(`   countActiveEmployees(null, '2026', null): ${testCount1}`);
        
        const testCount2 = await countActiveEmployees(null, '2026', targetUnit);
        console.log(`   countActiveEmployees(null, '2026', '${targetUnit}'): ${testCount2}`);
