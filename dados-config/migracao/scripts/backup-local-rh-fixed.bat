@echo off
REM 🔐 BACKUP SEGURO POSTGRESQL - RH+ LOCAL (Windows Fixed)
REM Uso: backup-local-rh-fixed.bat
REM Data: 28/04/2026
...
REM Adicionar PostgreSQL ao PATH temporariamente
set PATH=%PSQL_PATH%;%PATH%

REM Verificar se psql está acessível
psql --version >nul 2>&1
if errorlevel 1 (
    echo ❌ psql não encontrado em: %PSQL_PATH%
    pause
    exit /b 1
)
...
if errorlevel 1 (
    ...
    echo Verifique se o banco '%DB_NAME%' existe e o usuário '%DB_USER%' tem permissão.
    pause
    exit /b 1
)
...
if errorlevel 1 (
    ...
    echo Isso pode acontecer se gzip não estiver no PATH.
    echo O backup SQL foi criado com sucesso: %BACKUP_FILE%
) else (
...
if exist "%BACKUP_FILE%" (
    ...
    echo 📊 Tamanho do arquivo SQL: %SQL_SIZE% bytes
    
    if exist "%BACKUP_GZ%" (
        for %%A in ("%BACKUP_GZ%") do set GZ_SIZE=%%~zA
        echo 📊 Tamanho do arquivo GZ: %GZ_SIZE% bytes
        echo ✅ Backup verificado com sucesso
    ) else (
        echo ⚠️ Apenas backup SQL criado (compactado falhou)
    )
) else (
